...
@Injectable()
export class PolicyUpdateUseCases {
  constructor(
  ...
  async checkPolicyByName(policyName: string) {
    const resource = await this.policyRepository.find({ policyName });
  ...
  async updatePolicy(
    policyName: string,
    userId: number,
    polictDTO: PolicyCreateInputDTO,
  ) {
    try {
      const getPolicy = await this.verifyPolicyByName(policyName);
      if (getPolicy) {
        this.logger.log(
          'Success Find Policy',
